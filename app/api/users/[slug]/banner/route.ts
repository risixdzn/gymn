    ...
    try {
        ...
        if (isUuid) {
            ...
            const imageResponse = await fetch(
                `https://bkeiikprhrwohskvmxkd.supabase.co/storage/v1/object/public/banners/${
                    data.username
                }?timestamp=${Date.now()}`
            );
            ...
            return NextResponse.json({ success: false, error: "No banner found" }, { status: 404 });
        }

        const imageResponse = await fetch(
            `https://bkeiikprhrwohskvmxkd.supabase.co/storage/v1/object/public/banners/${slug}?timestamp=${Date.now()}`
        );
        // Check if the image was fetched successfully
        if (imageResponse.ok) {
            // Get the image buffer
